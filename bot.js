...
client.on('ready', () => {
  client.user.setGame(`$help |klay`,'https://www.twitch.tv/v5bz');
  console.log('---------------');
  console.log('---------------')
});
...
if (message.content === '$help') {
              var embed  = new Discord.RichEmbed()
				...
				.addField("تم انشاء خصائص جديده ")
				.addField("تم اضافة مانع الاسبام ويعطيه ميوت ويمسح الرسائل الاسبام   ")
				.addField("**$marry**,لعبة بسيطه :D  ")
				.addField("**$createroles**, يسويلك رتب ")		
				.addField("**$serooms**, يسويلك رومات ")
				.addField("$كف يعطي الشخص الي تمنشنه كف :joy:  ")
				.addField("الكود من تصميم ,@!NF iiklay,.&#4824")
                .setColor('RANDOM')
.setColor('RANDOM')
...
client.on('message', msg => {
  if (msg.content === '$invite') {
    msg.reply('https://discordapp.com/oauth2/authorize?client_id=473125268475871242&permissions=0&scope=bot');
  }
});
client.on('message', msg => {
  if (msg.content === '$help') {
...
client.on('message', msg => {
  if (msg.content === '!!!!') {
    msg.reply('ولكمممممم يابعد قلبي ارحب');
  }
  });
client.on('message', msg => {
...
client.on('message', msg => {
  if (msg.content === '$suppport') {
    msg.reply('السيرفر للمساعده,https://discord.gg/UFEJhcy');
  }
});
client.on("message", message => {
 ...
 client.on('message', message => {
  ...
  if(!message.member.hasPermission('ADMINISTRATOR')) return      message.channel.send('**للأسف لا تمتلك صلاحية** `ADMINISTRATOR`' );
    ...
    var bc = new
       ...
       .setImage("https://cdn.discordapp.com/attachments/453885998409973772/473408106903371808/46586e5f57duu75ef.png")
       .setThumbnail(message.author.avatarURL)
...
client.on('voiceStateUpdate', (u, member) => {
  var parent = '473497914484850698';
  var channel = '473498231095951380';
...
client.on('message', async message => {
    let args = message.content.split("RF");
    let command = args[0];
...
const prefix = "$";
  ...
  if(!message.guild.member(message.author).hasPermission("KICK_MEMBERS")) return message.reply("ليس عندي خصية ال kick").then(msg => msg.delete(5000));
  if(!message.guild.member(client.user).hasPermission("KICK_MEMBERS")) return message.reply("ليس عندي خصية ال kick");
...
client.on('message', message => {
    var args = message.content.split(/[ ]+/)
    if(message.content.includes('كل زق' ,'تفو عليك' ,'يا معفن' ,'يابن الكلب ','كسمك','يابن المتناكه','يابن الزنيه','يابن الزنوه')){
        message.delete()
...
client.on('message', message => {
    ...
    if(!message.channel.guild) return message.channel.send('**This Command Only For Servers !**')
            if (!message.member.hasPermission('MANAGE_ROLES')) return message.channel.send(`**${message.author.username} You Dont Have** ``MANAGE_ROLES`` **Premission**`);
					 ...
					 essage.guild.createRole({ name: "big boss", color: "#ffffff", permissions: [] })
					 message.guild.createRole({ name: "boss", color: "#ffffff", permissions: [] })
